...
## F 分支公司间 G05 收入进入 income_amt_assess 的排查点

2026-07-02 用 `yh_doc_cdm.dws_fin_income_cost_detail_assess_f`、`pt=20260701`、`sale_num=8001434498`、`comp_cd=50118843` 验证：

- 目标 F 表只有 1 行，`logic_flag='F生产公司-收入成本配比1'`，`data_type='材料销售-公司间'`，`income_amt_assess=-2415.93`，`from_dmbtr_dy=0`。
- 来源是 `dws_fin_income_cost_detail_assess_base_tmp` 中一条收入行：`vouch_type='G05'`，`subj_cd='G6001NB0206'`，`vouch_num='G0520260600244941'`，`vouch_line_proj='3'`，`vouch_amt=2415.93`，`remove_flag=1`，`biz_logic_flag='F'`，`income_cost_flag='income'`，`single_line_flag=0`。
- `dws_fin_income_cost_detail_assess_f` 的 `income_f` CTE 对 `income_amt_assess` 使用 `SUM(vouch_amt) * -1`，没有排除 `data_type='材料销售-公司间'`；但同一 CTE 中 `assess_vouch_amt` 明确要求 `base.data_type <> '材料销售-公司间'`，所以最终 `from_dmbtr_dy = income_f.assess_vouch_amt * -1` 为 0。
- `dws_fin_income_cost_detail_assess_base_tmp` 中曾有 `G05/K22` 剔除不参与配比的注释逻辑，但当前生产 SQL 里该条件被注释，`G05` 可以通过 `remove_flag=1` 进入 F 分支。

判断口径：如果业务确认 `材料销售-公司间` 不应贡献 F 表 `income_amt_assess`，根因在 F 表 `income_f.income_amt_assess` 公式未排除公司间数据，而不是目标行的 `from_dmbtr_dy` 或 ADS 展示层。修复应在 `dws_fin_income_cost_detail_assess_f` 的 `income_f` CTE 中对 `income_amt_assess` / `income_amt_assess_with_tax` 同步加入公司间排除，或在 base_tmp 恢复/新增 `G05` 公司间剔除规则；两种方案的影响范围不同，需要先做全量影响评估。
