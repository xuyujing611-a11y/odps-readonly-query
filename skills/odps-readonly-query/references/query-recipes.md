...
2. If `status` is `ok`, report `partition_value` and `row_cnt`.
3. If `status` is `ambiguous`, stop and report `candidates_by_token_index`. Do not choose a token.
4. If the user gives an exact date, run:
