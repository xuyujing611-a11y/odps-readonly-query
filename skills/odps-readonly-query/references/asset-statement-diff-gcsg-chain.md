...
`dws_asset_statement_diff_summary_gcsg` 的 `asset_sum.receivablebalance` 不是只取授信/借款余额，也会把所有 `account_type_name IN ('授信还款','借款还款','现款')` 的资产流水按 `amount * -1` 计入资产侧应收。因此，项目现款账户中的负数支出会增加资产侧应收，可能导致 `应收金额(receivablebalance)` 出现“资产比对账单多”的差异。

排查 `receivablebalance` 差异时，建议先按资产账号和 WBS 复算资产侧贡献：
...
SELECT account_type_name,
       ...
       SUM(CASE
               WHEN account_type_name IN ('授信使用','借款使用') THEN amount
               WHEN account_type_name IN ('授信还款','借款还款','现款') THEN amount * -1
               ELSE 0
           END) AS receivable_contribution
FROM yh_doc_cdm.dwd_mks_asset_detail_merge_for_account_statement
WHERE pt = '<bizdate>'
  AND cust_asset_acct = '<asset_account>'
  AND wbs_project_num = '<project_wbs_num>'
  AND contract_code = '<contract_code>'
  AND contract_2_code IN ('GS','ZS')
  AND REPLACE(SUBSTR(transaction_date,1,10),'-','') <= '<bizdate>'
GROUP BY account_type_name, sub_account_type_name, zc_glzc;
```

2026-07-02 已验证案例：

- 实体：合同 `YH30-GS-2022-0144`，WBS `DL-G0220600`，资产账号 `1468850996762198018`，分区 `pt=20260701`。
- ADS/DWS 差异：对账单侧 `receivablebalance=108762.65`，资产侧 `receivablebalance=122193.84`，差异 `-13431.19`（对账单 - 资产）。
- 直接差异单据：`yh_doc_cdm.dwd_mkt_cust_asset_detail.id=1684803932552347650`，具体资产账号 `ZH22122800213`，`source_num=YH30-GS-2022-0144`，`account_type_name=现款`，`transaction_source_type_name=工程垫资税金`，`transaction_type_name=支出`，`transaction_date=2023-07-28 00:00:00`，`transaction_amt=-13431.19`，状态 `启用`。
- 传递到合并资产明细后，对应 `yh_doc_cdm.dwd_mks_asset_detail_merge_for_account_statement` 行为 `sub_account_type_name=工程垫资税金`、`zc_glzc=支出`、`amount=-13431.19`；按资产应收公式贡献 `+13431.19`。
- 只读复算验证：资产侧全量应收贡献 `122193.84`；剔除该唯一 `工程垫资税金/支出/-13431.19` 行后为 `108762.65`，与对账单项目级应收完全一致。

这个案例只证明该差异由现款支出计入资产应收造成；是否应调整业务流水或调整 DWS `receivablebalance` 口径，需要业务确认后再修改生产 SQL。
...
- 只读 SQL：2026-07-02 对 `pt=20260701` 复算，合同 `YH30-GS-2022-0144`、WBS `DL-G0220600` 的应收差异 `-13431.19` 由资产明细 `1684803932552347650` 单行造成；剔除后资产侧应收与对账单侧应收均为 `108762.65`。
