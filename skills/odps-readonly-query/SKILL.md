...
- Never use a local cached SQL file as the edit baseline. First discover candidates with `trace-table <qualified_table>`, then fetch the intended PROD node with `trace-table <qualified_table> --node-id <node_id> --require-single-node --save-node-code <temp_dir> --compact-node-code`. Verify `node_id`, `project_id`, `connection`, `file_type`, `matched_output`, `project_env`, `node_role`, and `node_code_path` before editing.
...
python .\scripts\gateway_query.py --json trace-table <qualified_table> --node-id <node_id> --require-single-node --save-node-code outputs\node_code --compact-node-code
...
When `trace-table` returns multiple same-named nodes, do not substitute a similarly named table or `_da` node. Select the intended producer explicitly with `--node-id` (optionally also `--project-id`, `--connection`, `--file-type`, or `--matched-output`) and use `--require-single-node`. Saved SQL filenames include project id, node id, and file type so producer and synchronization nodes cannot overwrite one another.
